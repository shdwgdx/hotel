...
@section('content')
            ...
            <div class="row">
                <div class="col-12 col-lg-6">
                    ...
                    <div class="adress_hotel">
                        ...
                        <div class="adress_hotel_txt">
                            @if (__('main.select_lang') == 'en')
                                {{ $advertisement->adress_en }}
                            @endif
                            @if (__('main.select_lang') == 'es')
                                {{ $advertisement->adress_es }}
                            @endif
                            @if (__('main.select_lang') == 'ru')
                ...
                <div class="col-12 col-lg-6">
                    <div class="row_info_hotel">
                        <div class="item_info_hotel guests_icon">
                            {{ $advertisement->guests }} Guests
                        </div>
                        <div class="item_info_hotel bedroom_icon">
                            {{ $advertisement->bedrooms }} Bedroom
                        </div>
                        <div class="item_info_hotel bedroom_icon">
                            {{ $advertisement->bathroom }} Bedroom
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-12 col-md-4 col-lg-4">
                    <div class="body_dashbord_hotel" id="dashbord_hotel">
                        ...
                        <div class="price_value">€{{ $advertisement->price }} <span>/ ночь</span></div>
    ...
    <script>
        ...
        let calendar = flatpickr("#calendar", {
            ...
            minDate: "today",
            enable: [
                @foreach ($advertisement['dateList'] as $item)
                    {
                        from: "{{ \Carbon\Carbon::parse($item->start_active_adv)->format('d-m-Y') }}",
                        to: "{{ \Carbon\Carbon::parse($item->end_active_adv)->format('d-m-Y') }}"
                    },
                @endforeach
            ],
            static: true,
            "locale": '{{ __('main.select_lang') }}',
            onOpen: function() {
            ...
            onChange: function(selectedDates, dateStr, instance) {
                daysLength = dateStr.split('to')
                startDate = dateStr.slice(0, 10);

                endDate = dateStr.slice(13, 24);
                endDateElement = document.getElementById('endDate');
                StartDateElement = document.getElementById('calendar');
                endDateElement.value = endDate;
                StartDateElement.value = startDate;
                end_booked.value = endDate
                start_booked.value = startDate
                all_data_range_booked.value = startDate + ' - ' + endDate;

                let listRangeEnable = instance.config._enable;
                listRangeEnable.forEach(element => {
                    startDateTypeDate = instance.parseDate(startDate)
                    if (startDateTypeDate >= element.from && startDateTypeDate <= element.to) {
                        start_range_adv.value = element.from.toLocaleDateString();
                        end_range_adv.value = element.to.toLocaleDateString();
                        // console.log(element.from.toLocaleDateString());
                        // debugger
                    }
                ...
                if (daysLength.length > 1) {
                    daysLengthTotal = daysInRange.length + 2;
                    price = document.querySelectorAll('.body_dashbord_hotel__full_price')[0];
                    count_days = document.querySelectorAll('#count_day')[0];
                    price.innerHTML = "€" + parseFloat(daysLengthTotal * {{ $advertisement->price }})
                        .toFixed(
                            2);
                    count_days.innerHTML = daysLengthTotal;
                } else {
                    price = document.querySelectorAll('.body_dashbord_hotel__full_price')[0];
                    count_days =
                        document.querySelectorAll('#count_day')[0];
                    price.innerHTML = "€" + parseFloat(1 *
                            {{ $advertisement->price }})
                        .toFixed(
                            2);
                    count_days.innerHTML = "1";
                }
