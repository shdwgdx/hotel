...
class ProfileController extends Controller
{
    ...
    public function advertisements(Request $request)
    {
        ...
        $advertisemens = Advertisement::where('user_id', auth()->id())->orderBy('moderated', 'desc')->orderBy('published', 'asc')->orderBy('created_at', 'desc')->paginate(2);
        // $advertisemens = Advertisement::where('user_id', auth()->id())->where('published', 0)->where('moderated', 0)->whereOr('moderated', 1)->orderBy('created_at', 'desc')->paginate(2);
        $photos = Photo::get();
        ...
        return view('Profile.advertisements', compact('advertisemens',));
    }

    public function activeAdvertisements(Request $request)
    {

        // $advertisemens = Advertisement::where('user_id', auth()->id())->whereDate('expired_date', '>', now()->setTimezone('Europe/Moscow'))->orderBy('created_at', 'desc')->paginate(2);
        $advertisemens = Advertisement::where('user_id', auth()->id())->whereDate('expired_date', '>', now())->orderBy('created_at', 'desc')->paginate(2);
        $photos = Photo::get();
        for ($i = 0; $i < count($advertisemens); $i++) {
            $idAdvertisement = $advertisemens[$i]->id;
            $arrPhoto = Advertisement::join('photo_advertisement', 'photo_advertisement.id_advertisement', '=', 'advertisements.id')->select('url')->where('photo_advertisement.id_advertisement', '=', $idAdvertisement)->get();
        ...
        return view('Profile.active-advertisements', compact('advertisemens',));
    }


    public function inactiveAdvertisements(Request $request)
    {
        ...
        return view('Profile.inactive-advertisements', compact('advertisemens',));
    }
    ...
    {
        if (!Gate::allows('show-advertisement', $id)) {
            abort(403,);
        }
        ...
        $categories = Category::whereNull('parent_id')->get();
        return view('Profile.edit', compact('advertisemens', 'categories'));
    }
    ...
    public function republish(Request $request)
    {
        ...
        $advertisement->update(['expired_date' => now()->addDays(30)]);
    ...
    public function createApi(Request $request)
    {
        ...
        if ($verificated) {
            $images = $_FILES;
            if (count($images) < 1) {
                return json_encode(["status" => 'Нет изображений', "error" => 'no images']);
            }
            $num_adv = Advertisement::where('user_id', auth()->id())
                ->where('category_id', $request->category_id)
                ->count();
            if ($num_adv >= 2) {
                return response()->json(['status' => 'Вы не можете создать больше 2 объявлений в одной категории'],);
            }
            ...
            $result = json_encode(["status" => "Не подтверждена почта"]);
            return $result;
        }
        // dd($request);
